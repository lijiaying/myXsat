...
def _gen(expr_z3,symbolTable,cache,result):
    ...
    if _is_value(expr_z3):
        ...
        elif _is_true(expr_z3): str_ret= "0"
        elif _is_false(expr_z3): str_ret= "1"
        else: raise NotImplementedError("[XSat: Coral Benchmarking] type not considered ")
    ...
    if  sort_z3 == z3.Z3_OP_FPA_TO_FP: 
        ...
        toAppend= "%s %s = %s; " %( expr_type, var_name(expr_z3), x,)
        result.append(toAppend)
        return var_name(expr_z3)

    # compare operations:
    if sort_z3==z3.Z3_OP_FPA_LE or sort_z3==z3.Z3_OP_FPA_LT or _is_eq(expr_z3):
        ...
        if sort_z3==z3.Z3_OP_FPA_LE:
            ...
            toAppend= "double %s = DLE(%s,%s); " %(var_name(expr_z3), lhs, rhs,)
        elif sort_z3==z3.Z3_OP_FPA_LT:
            if DEBUG: print "-- Branch _is_lt"
            toAppend= "double %s = DLT(%s,%s);" %(var_name(expr_z3), lhs, rhs,)
        elif _is_eq(expr_z3):
            if DEBUG: print "-- Branch _is_eq"
            toAppend= "double %s = DEQ(%s,%s);" %(var_name(expr_z3), lhs, rhs,)
        result.append(toAppend)
        return var_name(expr_z3)
        
    # arithmetic operations:
    if _is_fpAdd(expr_z3) or _is_fpMul(expr_z3) or _is_fpDiv(expr_z3):
        if not _is_RNE(expr_z3.arg(0)):
        ...
        if _is_fpAdd(expr_z3):
            ...
            toAppend= "%s %s = %s+%s;" %(expr_type, var_name(expr_z3), lhs, rhs,)
        elif _is_fpMul(expr_z3):
            if DEBUG: print "-- Branch _is_fpMul"
            toAppend= "%s %s = %s*%s; " %(expr_type, var_name(expr_z3), lhs, rhs,)
        elif _is_fpDiv(expr_z3):
    ...
    if _is_fpNeg(expr_z3):
        ...
        toAppend = "%s %s =  - %s ;" %(expr_type, var_name(expr_z3), op1,)
        result.append(toAppend)
        return var_name(expr_z3)
    ...
    if z3.is_and(expr_z3):
        ...
        toAppend= "double %s = %s; " %(var_name(expr_z3), toAppendExpr,)
        result.append(toAppend)
        return var_name(expr_z3)
    ...
    if z3.is_not(expr_z3):
        ...
        if _is_ge(expr_z3.arg(0)): a= "DLT(%s,%s)" %(op1,op2)
        elif _is_gt(expr_z3.arg(0)): a= "DLE(%s,%s)" %(op1,op2)
        elif _is_le(expr_z3.arg(0)): a= "DGT(%s,%s)" %(op1,op2)
        elif _is_lt(expr_z3.arg(0)): a= "DGE(%s,%s)" %(op1,op2)
        elif _is_eq(expr_z3.arg(0)): a= "DNE(%s,%s)" %(op1,op2)
        elif _is_distinct(expr_z3.arg(0)): a= "DEQ(%s,%s)" %(op1,op2)
        else: raise NotImplementedError      ("Not implemented case 004 for expr_z3  =  %s. 'Not(or... )' is not handled yet" %expr_z3)
        toAppend= "%s %s = %s; " %(expr_type, var_name(expr_z3), a,)
        result.append(toAppend)
        return var_name(expr_z3)
